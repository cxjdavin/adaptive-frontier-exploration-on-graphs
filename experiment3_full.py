...
def get_result_pickle_filename(std_name: int) -> str:
    return f"results/exp3_{std_name}_full.pkl"

def get_graph_pickle_filename(std_name: str) -> str:
    return f"results/exp3/graphs/{std_name}_full.pkl"
...
def run_experiment4(std_name: int, experiment_result_pickle_filename: str, learning_rate: float = 0.001, fit_theta: bool = False) -> None:
    multithread = True
    train_policies = False
    num_monte_carlo_runs = 200
    rng_seed = 42
    beta = 0.99
    policy_indices = [0, 1, 3]
    ...
    G, covariates, statuses = processor.merged_datasets[std_name]

    all_inst_configs = []
    G_pickle_filename = get_graph_pickle_filename(std_name)
    ...
    inst_configs['instance_hash'] = f"exp3_{std_name}_full_{learning_rate}_{beta}_{num_monte_carlo_runs}_{rng_seed}"
    ...
    print(f"Solving {std_name}, full disease graph")
...
def plot(experiment_result_pickle_filename: str) -> None:
    ...
    plot_filename = f"results/plots/exp3_{stats['std_name']}_full.png"
    ...
    plt.savefig(f"results/plots/exp3_{std_name}_full_graph.png", dpi=300, bbox_inches = 'tight')
...
if __name__ == "__main__":
    ...
    print(mode, std_idx, learning_rate)
    ...
    experiment_result_pickle_filename = get_result_pickle_filename(std_name)

    if not just_plot:
        run_experiment4(std_name, experiment_result_pickle_filename, learning_rate, fit_theta)
